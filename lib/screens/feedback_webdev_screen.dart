...
class WebDevFeedbackScreen extends StatelessWidget {
  final FeedbackResult result;

  const WebDevFeedbackScreen({required this.result, super.key});
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(
              ...
              child: Column(children: [
                ...
                const SizedBox(height: 14),
                SizedBox(width: 160, height: 160, child: Stack(alignment: Alignment.center, children: [SizedBox(width: 160, height: 160, child: CircularProgressIndicator(value: result.overall / 100.0, backgroundColor: Colors.white12, color: const Color(0xFF8F67FF), strokeWidth: 12)), Column(mainAxisSize: MainAxisSize.min, children: [Text(result.overall.toInt().toString(), style: const TextStyle(fontSize: 42, fontWeight: FontWeight.w800, color: Color(0xFFB4A7FF))), const SizedBox(height: 4), Text('/${result.scaleMax}', style: TextStyle(color: Colors.white.withValues(alpha: 0.75), fontSize: 20))])])),
                const SizedBox(height: 12),
                if (result.ratingLabel != null) ...[Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6), decoration: BoxDecoration(color: const Color(0xFF2B243D), borderRadius: BorderRadius.circular(999)), child: Text(result.ratingLabel!, style: const TextStyle(color: Color(0xFFB4A7FF), fontWeight: FontWeight.w700))), const SizedBox(height: 12)],
            ...
            const SizedBox(height: 12),
            GridView(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 2.15, crossAxisSpacing: 12, mainAxisSpacing: 12), children: [card('Technical Knowledge', result.scores['Technical Knowledge'] ?? 0.0, Icons.code_rounded, const Color(0xFF8F67FF)), card('Problem Solving', result.scores['Problem Solving'] ?? 0.0, Icons.lightbulb_outline_rounded, const Color(0xFFA7E45B)), card('Communication', result.scores['Communication'] ?? 0.0, Icons.chat_outlined, const Color(0xFFF5A34A)), card('Code Quality', result.scores['Code Quality'] ?? 0.0, Icons.tune_rounded, const Color(0xFF74A7FF))]),
